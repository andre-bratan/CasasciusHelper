...
public class KeyPair : PublicKey
{
    ...
    protected override byte[] ComputePublicKey()
    {
        ...
        var noramlized = dd.Normalize();
        if (IsCompressedPoint)
        {
            dd = ps.Curve.CreatePoint(noramlized.AffineXCoord.ToBigInteger(), noramlized.AffineYCoord.ToBigInteger());
            return dd.GetEncoded(true);
        }
        else
        {
            byte[] pubaddr = new byte[65];
            byte[] Y = noramlized.AffineYCoord.ToBigInteger().ToByteArray();
            Array.Copy(Y, 0, pubaddr, 64 - Y.Length + 1, Y.Length);
            byte[] X = noramlized.AffineXCoord.ToBigInteger().ToByteArray();
    ...
    {
        get { return Util.ByteArrayToString(PrivateKeyBytes); }
