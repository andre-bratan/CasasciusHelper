...
public class PublicKey : AddressBase
{
    ...
    {
        this.IsCompressedPoint = true; // Ported code: assuming points are always compressed by default - was "point.IsCompressed;"
        this.point = point;
    ...
    private bool validatePoint()
    {
        ...
        var y2 = normalized.AffineYCoord.Multiply(normalized.AffineYCoord);
        var x3 = normalized.AffineXCoord.Multiply(normalized.AffineXCoord).Multiply(normalized.AffineXCoord);
        var ax = normalized.AffineXCoord.Multiply(ps.Curve.A);
    ...
    /// </summary>
    protected virtual byte[] ComputePublicKey()
    {
        return null;
    }
    ...
    private byte[] getReencoded(bool compressed)
    {
        ...
        point = ps.Curve.DecodePoint(pubKeyBytes);
        var normalized = point.Normalize();
        var encoded = normalized.GetEncoded(compressed);
        return encoded;
    }

    public static ECPoint GetUncompressed(ECPoint point)
    {
        var normalized = point.Normalize().GetEncoded(false);
        var ps = Org.BouncyCastle.Asn1.Sec.SecNamedCurves.GetByName("secp256k1");
        var result = ps.Curve.DecodePoint(normalized);
        return result;
    }

    public static ECPoint GetCompressed(ECPoint point)
    {
        var normalized = point.Normalize().GetEncoded(true);
        var ps = Org.BouncyCastle.Asn1.Sec.SecNamedCurves.GetByName("secp256k1");
        var result = ps.Curve.DecodePoint(normalized);
        return result;
    }
    ...
    public string PublicKeyHex
    {
        ...
        get { return Util.ByteArrayToString(PublicKeyBytes); }
    }
}
